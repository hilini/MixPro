...
imagenet_path="data/imagenet"  # 예: ImageNet 데이터셋 경로
batch_size=128  # 예: 각 GPU에 대한 배치 크기
output_directory="deit_results"  # 예: 결과를 저장할 디렉터리
...
--cfg $config_file --data-path $imagenet_path --batch-size $batch_size --output $output_directory --tag $job_tag
 
 python -m torch.distributed.launch --nproc_per_node 1 --master_port 14  main.py \
--cfg configs/deit/deit_tiny_patch14_mask56_224_alpha1.yaml --data-path /data/imagenet --batch-size 128 --output output --tag 1004 